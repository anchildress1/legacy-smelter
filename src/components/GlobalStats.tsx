...
export const GlobalStats: React.FC<GlobalStatsProps> = ({ totalPixels }) => {
  ...
  return (
    <div className="modern-card p-6 relative overflow-hidden">
      ...
      <div className="mt-2">
        <h3 className="text-steel-blue font-mono text-xs uppercase tracking-widest mb-1">
          GLOBAL SMELT ACCUMULATION
        </h3>
        <div className="neon-label text-4xl font-extrabold font-mono text-acid-green tracking-tighter">
          {formatted.value}
          <span className="text-sm ml-2 text-zinc-400">{formatted.unit}</span>
        </div>
      </div>
      <div className="mt-4 flex gap-2 items-center">
        <div className="w-2 h-2 rounded-full bg-acid-green animate-pulse" />
        <div className="text-[10px] font-mono text-acid-green uppercase">
